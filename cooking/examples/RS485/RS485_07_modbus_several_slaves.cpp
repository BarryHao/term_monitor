...
// *****************************************************************************************
  ...
  if (result != 0) {
    ...
    printf("Communication error. Couldn't read from temperature sensor");
    delay(100);
  } 
  else { 

    // If all OK
    printf("Temperature read from slave ID 1: %d\n",temperatureSensor.getResponseBuffer(0));
...
// *****************************************************************************************
  ...
  if (result != 0) {
    // If no response from the slave, print an error message 
    printf("Communication error. Couldn't read from the C02 sensor\n");
    delay(100);
  } 
  else { 

    // If all OK
    printf("CO2 data read from slave ID 2: %d\n",co2Sensor.getResponseBuffer(0));
